...
serve(async (req) => {
  ...
  try {
  const { email, discountCode, pdfBase64, pdfName } = await req.json()
    ...
    const brevoApiKey = Deno.env.get("VITE_BREVO_API_KEY") || Deno.env.get("BREVO_API_KEY");
    const brevoRes = await fetch("https://api.brevo.com/v3/smtp/email", {
      ...
      body: JSON.stringify({
        sender: { email: "info@petstore.bg", name: "Bat Petko Store" },
        to: [{ email }],
